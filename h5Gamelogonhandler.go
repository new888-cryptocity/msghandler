...
//處理H5遊戲登入請求 (html5 game) (若第一次登入, 會註冊到 Users.UserInfo 表)
type H5GameLogonHandler struct {
}

func (h *H5GameLogonHandler) IsHook(bclsid int32, sclsid int32) bool {
	return bclsid == int32(netproto.MessageBClassID_DBServer) && sclsid == int32(netproto.DBServerClassID_H5GameLoginID)
}

func (h *H5GameLogonHandler) OnMessage(ctx *module.MouduleCtx, clt *network.ConnInfo, m *msg.Message) []*msg.Message {
	...
	mlog.Debug("[%s] thirdPartyUserAmount=%v", proName, thirdPartyUserAmount)
